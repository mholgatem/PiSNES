		...
		case SDL_JOYAXISMOTION:
			if(event.jaxis.axis == joyaxis_LR_1) {
				if(event.jaxis.value > -10000 && event.jaxis.value < 10000)
					joy_axes[joyaxis_LR_1] = CENTER;
				else if(event.jaxis.value > 10000)
					joy_axes[joyaxis_LR_1] = RIGHT;
				else
					joy_axes[joyaxis_LR_1] = LEFT;
			}
			if(event.jaxis.axis == joyaxis_UD_1) {
				if(event.jaxis.value > -10000 && event.jaxis.value < 10000)
					joy_axes[joyaxis_UD_1] = CENTER;
				else if(event.jaxis.value > 10000)
					joy_axes[joyaxis_UD_1] = DOWN;
				else
					joy_axes[joyaxis_UD_1] = UP;
			}
			break;
		case SDL_KEYDOWN:
...
static uint32 fe_ReadJoypad (int which1)
{
	...
	if (keyssnes[sfc_key[UP_1]] == SDL_PRESSED || joy_axes[joyaxis_UD_1] == UP)		val |= SNES_UP_MASK;
	if (keyssnes[sfc_key[DOWN_1]] == SDL_PRESSED || joy_axes[joyaxis_UD_1] == DOWN)	val |= SNES_DOWN_MASK;
	if (keyssnes[sfc_key[LEFT_1]] == SDL_PRESSED || joy_axes[joyaxis_LR_1] == LEFT)	val |= SNES_LEFT_MASK;
	if (keyssnes[sfc_key[RIGHT_1]] == SDL_PRESSED || joy_axes[joyaxis_LR_1] == RIGHT)	val |= SNES_RIGHT_MASK;
...
/*	sfc_key[LEFT_2] = SDLK_4;
	...
	sfc_joy[A_1] = get_keyjoy_conf("Joystick", "A_1", RPI_JOY_A_1);
	sfc_joy[B_1] = get_keyjoy_conf("Joystick", "B_1", RPI_JOY_B_1);
	sfc_joy[X_1] = get_keyjoy_conf("Joystick", "X_1", RPI_JOY_X_1);
	sfc_joy[Y_1] = get_keyjoy_conf("Joystick", "Y_1", RPI_JOY_Y_1);
	sfc_joy[L_1] = get_keyjoy_conf("Joystick", "L_1", RPI_JOY_L_1);
	sfc_joy[R_1] = get_keyjoy_conf("Joystick", "R_1", RPI_JOY_R_1);
	sfc_joy[START_1] = get_keyjoy_conf("Joystick", "START_1", RPI_JOY_START_1);
	sfc_joy[SELECT_1] = get_keyjoy_conf("Joystick", "SELECT_1", RPI_JOY_SELECT_1);
